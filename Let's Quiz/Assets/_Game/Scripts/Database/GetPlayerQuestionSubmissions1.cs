...
namespace _LetsQuiz
{
	public class GetPlayerQuestionSubmissions : MonoBehaviour
	{
		...
		private void handleData(string json)
		{
			...
			qsc = JsonUtility.FromJson<QuestAndSubContainer> (json);
			//now we have an object that holds our array of questAndSub objects
			QuestAndSub[] _questAndSub = qsc.dataForQuestAndSub;
			Debug.Log(_questAndSub[0].QuestionText);
